  ...
  Podcast.findOne({ podcastId: podcastId }).then((podcast) => {
    if (podcast) {
      if (podcast.author !== username) {
        res
          .status(400)
          .json({ message: 'Non Autorizzato ad eliminare questo podcast.' });
      } else {
        Podcast.findOneAndDelete({ podcastId: podcastId })
          .then((podcast) => {
            //here now we remove the file
            fs.unlink(
              path.join(__dirname, '../podcasts', podcastId + '.mp3'),
              (err) => {
                if (err) {
                  res.status(400).json({ message: 'Qualcosa e andato storto' });
                } else {
                  res
                    .status(200)
                    .json({ message: 'Podcast eliminato con successo.' });
                }
              }
            );
          })
